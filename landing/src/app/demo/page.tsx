...
export default function DemoPage() {
  ...
  const [activeTab, setActiveTab] = useState<"orders" | "products" | "stores">(
    "orders"
  );
      ...
      <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur-xl border-b border-slate-200/60 shadow-sm">
        <div className="max-w-7xl mx-auto px-5 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] font-black uppercase tracking-widest bg-amber-500 text-white px-2 py-0.5 rounded-lg">
              DEMO
            </span>
          </div>
      ...
      <main className="max-w-7xl mx-auto px-5 py-12 space-y-16">
        ...
        <motion.div {...fadeInUp} className="text-center space-y-6 py-8">
          ...
          <p className="text-xl text-slate-500 max-w-2xl mx-auto leading-relaxed">
            Explore the full B2B distribution platform — real data, real UI. No
            demo account needed.
          </p>
        </motion.div>
        ...
        {loading ? (
          ...
          >
            {[
              {
                label: "Companies",
                value: stats?.totalCompanies?.toLocaleString() || "134",
                icon: Building2,
                color: "text-blue-600",
                bg: "bg-blue-50",
              },
              {
                label: "Total Orders",
                value: stats?.totalOrders?.toLocaleString() || "18,492",
                icon: ShoppingCart,
              ...
              {
                label: "Revenue (UZS)",
                value: stats?.totalRevenue || "47.8B",
                icon: TrendingUp,
              ...
              {
                label: "Active Users",
                value: stats?.activeUsers?.toLocaleString() || "312",
                icon: Users,
              ...
              {
                label: "API Response",
                value: `${stats?.avgResponseMs || 82}ms`,
                icon: Zap,
                color: "text-cyan-600",
                bg: "bg-cyan-50",
              },
              ...
              >
                <div
                  className={`w-10 h-10 rounded-xl ${s.bg} flex items-center justify-center mx-auto mb-3`}
                >
        ...
        <motion.div
          {...fadeInUp}
          className="bg-white rounded-3xl border border-slate-100 overflow-hidden shadow-xl shadow-slate-200/30"
        >
          {/* Tab header */}
          <div className="border-b border-slate-100 p-6 flex flex-col sm:flex-row gap-4 sm:items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-400 mt-1">
                Sample data from a demo distribution company
              </p>
            </div>
                ...
                <tbody>
                  {orders.map((order) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`px-2.5 py-1 rounded-lg text-[10px] font-black uppercase tracking-wide ${STATUS_COLOR[order.status] || "bg-slate-50 text-slate-500"}`}
                        >
                          {order.status}
                        </span>
                      </td>
                ...
                <tbody>
                  {products.map((product) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <span
                          className={`px-2.5 py-1 rounded-lg text-[10px] font-black ${product.stock > 100 ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"}`}
                        >
          ...
          {activeTab === "stores" && (
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
              {stores.map((store) => (
                <div
                  key={store.id}
                  className="bg-slate-50 rounded-2xl p-6 border border-slate-100 space-y-4 hover:border-blue-200 transition-all group"
                >
                  <div className="flex items-center gap-4">
        ...
        >
          {[
            ...
            >
              <div
                className={`w-12 h-12 rounded-2xl ${f.bg} flex items-center justify-center group-hover:scale-110 transition-transform`}
              >
          ...
          <motion.div {...fadeInUp} className="space-y-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-5">
              {tariffs.map((plan) => (
                <div
                  key={plan.id}
                  className={`bg-white p-7 rounded-2xl border ${plan.isPopular ? "border-blue-500 ring-2 ring-blue-500/20" : "border-slate-100"} space-y-6 relative hover:shadow-xl transition-all`}
                >
                  {plan.isPopular && (
          ...
          {[
            {
              ...
              desc: "Per-tenant data isolation",
              color: "text-blue-600",
              bg: "bg-blue-50",
            },
            {
            ...
            },
            {
              icon: Star,
              label: "5-Language UI",
              desc: "UZ, EN, RU, TR, UZ_CYRL",
              color: "text-amber-600",
              bg: "bg-amber-50",
            },
            {
              icon: Zap,
              label: "<100ms Response",
            ...
            <div
              key={i}
              className="bg-white p-6 rounded-2xl border border-slate-100 text-center space-y-3"
            >
              <div
                className={`w-10 h-10 rounded-xl ${t.bg} flex items-center justify-center mx-auto`}
              >
        ...
        <motion.div
          {...fadeInUp}
          className="bg-slate-900 rounded-3xl p-12 text-center space-y-8 relative overflow-hidden"
        >
          ...
          <div className="relative z-10 space-y-4">
            ...
            <p className="text-slate-400 text-lg max-w-xl mx-auto leading-relaxed">
              Join 130+ companies already using Supplio to manage their dealer
              networks.
            </p>
          </div>
          <div className="flex flex-col sm:flex-row gap-4 justify-center relative z-10">
            ...
            <Link
              href="/uz"
              className="px-8 py-4 bg-white/10 text-white border border-white/20 rounded-2xl font-black hover:bg-white/20 transition-all flex items-center gap-3 justify-center"
            >
