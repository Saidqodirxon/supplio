...
const NEWS_DETAIL_TEXT = {
  ...
  ru: {
    ...
    ctaSubtitle: "Запустите 14-дневный премиум-тест. Банковская карта не нужна.",
    home: "Главная",
  ...
  en: {
    ...
    notFoundDesc: "The article you are looking for doesn't exist or was removed.",
...
function renderMarkdown(raw: string) {
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith('### ')) {
      flushList();
      nodes.push(<h3 key={key++} className="text-xl font-black text-slate-900 mt-8 mb-3 tracking-tight">{trimmed.slice(4)}</h3>);
    } else if (trimmed.startsWith('## ')) {
      flushList();
      nodes.push(<h2 key={key++} className="text-2xl font-black text-slate-900 mt-10 mb-4 tracking-tight border-b border-slate-100 pb-3">{trimmed.slice(3)}</h2>);
    } else if (trimmed.startsWith('# ')) {
      flushList();
      nodes.push(<h1 key={key++} className="text-3xl font-black text-slate-900 mt-10 mb-4 tracking-tight">{trimmed.slice(2)}</h1>);
    } else if (trimmed.startsWith('- ') || trimmed.startsWith('* ')) {
      listItems.push(trimmed.slice(2));
    } else {
      flushList();
      nodes.push(
        <p key={key++} className="text-slate-600 leading-relaxed font-medium mb-4"
...
export default function NewsDetailPage() {
  ...
  useEffect(() => {
    ...
    try {
      const saved = JSON.parse(savedRaw) as Array<{ id?: string; slug?: string }>;
      setIsSaved(saved.some((item) => item.id === article.id || item.slug === slug));
    } catch {
      setIsSaved(false);
    }
  }, [article, slug]);
  ...
  const handleToggleSave = () => {
    ...
    const savedRaw = localStorage.getItem("supplio:saved-news") || "[]";
    let saved: Array<{ id: string; slug: string; title: string; lang: string; savedAt: string }> = [];
    ...
    const exists = saved.some((item) => item.id === article.id || item.slug === slug);

    if (exists) {
      const next = saved.filter((item) => item.id !== article.id && item.slug !== slug);
    ...
    return (
      <div className="h-screen flex items-center justify-center font-outfit">
        <div className="flex flex-col items-center gap-4 text-slate-400">
          ...
          <p className="text-sm font-bold uppercase tracking-widest">{uiText.loading}</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="h-screen flex flex-col items-center justify-center font-outfit gap-8">
        <div className="text-center space-y-8 p-12 rounded-[3rem] border border-slate-100 max-w-sm bg-white shadow-xl">
          ...
          <h1 className="text-3xl font-black text-slate-900 tracking-tighter">{uiText.notFoundTitle}</h1>
          <p className="text-sm text-slate-400 font-medium">{uiText.notFoundDesc}</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-24 font-outfit">
            ...
            <aside className="space-y-16">
              <div className="bg-white p-10 rounded-[3rem] border border-slate-100 shadow-xl shadow-slate-200/20 space-y-10">
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{uiText.shareInsight}</p>
                <div className="flex flex-col gap-4">
                  ...
                  >
                    <span className="flex items-center gap-4"><Share2 className="w-5 h-5" /> {uiText.copyLink}</span>
                  ...
                  >
                    <span className="flex items-center gap-4">
                      {isSaved ? <BookmarkCheck className="w-5 h-5" /> : <Bookmark className="w-5 h-5" />} {isSaved ? uiText.saved : uiText.save}
                    </span>
                    <ChevronRight className="w-4 h-4 opacity-0 group-hover:opacity-100 transition-opacity" />
                  </button>
                </div>
              ...
              <div className="bg-slate-900 p-12 rounded-[4rem] text-white shadow-3xl shadow-slate-900/40 space-y-8 relative overflow-hidden group">
                ...
                <p className="text-3xl font-black leading-tight relative z-10 tracking-tighter">{uiText.ctaTitle}</p>
                <p className="text-sm font-bold text-slate-400 relative z-10 leading-relaxed">{uiText.ctaSubtitle}</p>
                <button
      ...
      <footer className="py-24 px-6 border-t border-slate-100 bg-white">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-10">
          ...
          <div className="flex gap-12 font-black items-center text-slate-400 text-[10px] uppercase tracking-[0.2em]">
            <Link href={`/${params.lang}`} className="hover:text-blue-600 transition-colors">{uiText.home}</Link>
            <Link href={`/${params.lang}#news`} className="hover:text-blue-600 transition-colors">{uiText.allArticles}</Link>
          </div>
        </div>
      </footer>
