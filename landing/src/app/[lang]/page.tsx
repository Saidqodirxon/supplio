...
export default function LandingPage() {
  ...
  const notification = settings
    ? settings[`globalNotify${lang.charAt(0).toUpperCase() + lang.slice(1)}`]
    : null;
  ...
  return (
    <div className="min-h-screen selection:bg-blue-600 selection:text-white overflow-x-hidden font-sans bg-white text-left">
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-6 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-5">
            ...
            <button
              ...
              className="px-5 py-2.5 bg-blue-600 text-white rounded-xl text-sm font-bold hover:bg-blue-700 transition-all shadow-lg shadow-blue-600/20 active:scale-95"
            >
              {t.nav.register}
            </button>
          </div>
      ...
      <AnimatePresence>
        {isMenuOpen && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="fixed inset-0 z-40 md:hidden pt-24 px-5 bg-white"
          >
      ...
      <section className="relative pt-32 pb-20 sm:pt-48 sm:pb-32 px-5 sm:px-6 overflow-hidden bg-slate-50">
        ...
        <div className="max-w-7xl mx-auto text-center relative">
          <motion.div
            ...
            className="inline-flex items-center gap-2.5 px-4 py-2 rounded-full bg-blue-50 border border-blue-100/50 text-blue-700 text-xs sm:text-sm font-bold mb-8"
          >
          ...
          <motion.h1
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="text-4xl sm:text-5xl md:text-7xl font-bold text-slate-900 leading-[1.1] tracking-tight mb-8"
          >
            {heroTitle}
          </motion.h1>
          ...
          >
            <button
              onClick={() => setIsLeadModalOpen(true)}
              className="w-full sm:w-auto px-8 py-4 bg-blue-600 text-white rounded-2xl font-bold text-base hover:bg-blue-700 transition-all shadow-xl shadow-blue-600/30 hover:scale-[1.02] active:scale-[0.98]"
            >
              {t.hero.cta}
            </button>
            <Link
        ...
        <div className="max-w-7xl mx-auto px-5 sm:px-6">
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12">
            {[
              ...
              {
                label: t.stats.isolation,
                value: t.stats.isolationValue,
      ...
      <section
        id="features"
        className="py-24 sm:py-32 px-5 sm:px-6 bg-white text-left"
      >
        <div className="max-w-7xl mx-auto">
          <motion.div
            {...fadeInUp}
            className="text-center md:text-left max-w-3xl mb-20"
          >
        ...
        <div className="max-w-7xl mx-auto">
          ...
          >
            <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-slate-900 tracking-tight mb-6">
              {howItWorksTranslations[lang]?.title ??
                howItWorksTranslations.en.title}
            </h2>
            <p className="text-lg text-slate-500 leading-relaxed font-medium">
              {howItWorksTranslations[lang]?.subtitle ??
                howItWorksTranslations.en.subtitle}
            </p>
          </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
            ...
            {(
              ...
              howItWorksTranslations.en.steps
            ).map((step, i) => (
              <motion.div
                key={i}
                {...fadeInUp}
            ...
            {testimonials.length <= 3 ? (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {testimonials.map((tm, i) => {
                  const contentKey =
                    `content${lang.charAt(0).toUpperCase() + lang.slice(1)}` as keyof typeof tm;
                  ...
                  return (
                    ...
                    >
                      ...
                      <p className="text-slate-700 leading-relaxed italic flex-1">
                        "{content}"
                      </p>
              ...
              <div className="relative">
                  ...
                  <AnimatePresence mode="wait">
                    {(() => {
                      ...
                      const pageItems = testimonials.slice(
                        testimonialIdx * perPage,
                        testimonialIdx * perPage + perPage
                      );
                      return (
                        <motion.div
                          key={testimonialIdx}
                        ...
                        >
                          {pageItems.map((tm, i) => {
                            const contentKey =
                              `content${lang.charAt(0).toUpperCase() + lang.slice(1)}` as keyof typeof tm;
                            const content =
                              (tm[contentKey] as string) || tm.contentEn;
                            return (
                              <div
                                key={tm.id}
                                className="flex flex-col gap-6 p-8 bg-slate-50 rounded-3xl border border-slate-100 hover:shadow-xl hover:border-blue-100 transition-all duration-300"
                              >
                                <div className="flex gap-1">
                                  {Array.from({ length: tm.rating }).map(
                                    (_, s) => (
                                      <svg
                                        key={s}
                                        className="w-5 h-5 text-amber-400 fill-amber-400"
                ...
                <div className="flex items-center justify-center gap-4 mt-10">
                  ...
                  <button
                    onClick={() =>
                      setTestimonialIdx((i) =>
                        Math.min(Math.ceil(testimonials.length / 3) - 1, i + 1)
                      )
                    }
                    disabled={
                      testimonialIdx >= Math.ceil(testimonials.length / 3) - 1
                    }
                    className="w-10 h-10 rounded-full border border-slate-200 flex items-center justify-center text-slate-500 hover:border-blue-600 hover:text-blue-600 disabled:opacity-30 transition-all"
                  >
          ...
          return (
            <div className="max-w-3xl mx-auto">
              ...
              <motion.button
                ...
                transition={{ delay: 0.2 }}
                onClick={() => setIsLeadModalOpen(true)}
                className="px-10 py-5 bg-white text-blue-600 rounded-2xl font-bold text-base hover:bg-blue-50 transition-all shadow-xl hover:scale-[1.02] active:scale-[0.98]"
              >
                {t.nav.register}
              </motion.button>
            </div>
      ...
      <section
        id="pricing"
        className="py-24 sm:py-32 px-5 sm:px-6 bg-slate-900 relative overflow-hidden text-left"
      >
        ...
        <div className="max-w-7xl mx-auto">
          ...
          {(() => {
            const plans = (dynamicTariffs.length > 0
              ? dynamicTariffs
            ...
            const PlanCard = ({ plan, i }: { plan: SafePlan; i: number }) => {
              ...
              const displayFeatures =
                dynamicTariffs.length > 0
                  ? [...(features || []), getBotFeature(maxCustomBots, lang)]
                  : features || [];
              ...
              return (
                <motion.div
                  {...fadeInUp}
                  transition={{ delay: i * 0.1 }}
                  className={`relative flex flex-col p-7 rounded-3xl border h-full ${isScrollable ? "min-w-75 w-75" : ""} ${isPopular ? "bg-white border-blue-600/20 shadow-2xl ring-2 ring-blue-600/30 z-10" : "bg-slate-800/40 border-slate-700/50 backdrop-blur-sm"}`}
                >
                  {isPopular && (
                  ...
                  <button
                    onClick={() => setIsLeadModalOpen(true)}
                    className={`block w-full py-4 rounded-xl font-bold text-sm text-center transition-all active:scale-[0.97] ${isPopular ? "bg-blue-600 text-white shadow-lg shadow-blue-600/25 hover:bg-blue-700" : "bg-white text-slate-900 hover:bg-slate-50"}`}
                  >
                    {t.pricing.cta}
                  </button>
                </motion.div>
      ...
      {settings?.newsEnabled !== false && (
        ...
        >
          <div className="max-w-7xl mx-auto">
            <motion.div
              {...fadeInUp}
              className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8 text-left"
            >
              ...
              {(dynamicNews as unknown as SafeNews[]).map((news, i: number) => {
                const lkNews =
                  lang === "oz"
                    ...
                    : lang.charAt(0).toUpperCase() + lang.slice(1);
                const title = String(
                  news[`title${lkNews}`] || news["titleEn"] || ""
                );
                const excerpt = String(
                  news[`excerpt${lkNews}`] || news["excerptEn"] || ""
                );
                const slug = String(
                  news[`slug${lkNews}`] || news["slugEn"] || ""
                );
                const date = news.createdAt
                  ? format(new Date(news.createdAt as string), "dd MMM yyyy")
                  : "";
                    ...
                    >
                      ...
                      <div className="relative w-full aspect-[16/9] bg-slate-100 overflow-hidden shrink-0">
                        {news.image ? (
                          <img
                            src={
                              String(news.image).startsWith("http")
                                ? String(news.image)
                                : `${BACKEND}${news.image}`
                            }
                            alt={title}
