...
export class TelegramService implements OnModuleInit {
  ...
  async initBot(
    ...
    try {
      ...
      // Global error handler — bot never stops, always replies with a safe message
      bot.catch(async (err: unknown, ctx: Context) => {
        const msg = err instanceof Error ? err.message : String(err);
        this.logger.error(`[Bot ${botId}] Unhandled error: ${msg}`);
        try {
          await (ctx as any).reply("⚠️ Xatolik yuz berdi. Qaytadan /start bosing yoki keyinroq urinib ko'ring.");
        } catch {}
      });
      ...
      bot.start(async (ctx) => {
        ...
        // Registered dealers: auto-detect language, skip language selection prompt
        // New users: show language selection if language not yet chosen
      ...
      // Fallback: handle any unrecognized text (reply keyboard leftovers, random messages)
  ...
  private async handleOrders(ctx: Context, companyId: string, editMsg = false) {
    ...
    if (orders.length === 0) {
      if (editMsg) {
        try {
          await (ctx as any).editMessageText(t.noOrders, {
            reply_markup: backOpts,
          });
          return;
        } catch {}
  ...
  private async handlePayments(
    ...
    if (payments.length === 0) {
      if (editMsg) {
        try {
          await (ctx as any).editMessageText(t.noPayments, {
            reply_markup: backOpts,
          });
          return;
        } catch {}
  ...
  private async handleCallback(ctx: Context, companyId: string) {
    ...
    const safeAnswer = async (text?: string, showAlert = false) => {
      try {
        await (ctx as any).answerCbQuery(text ?? "", { show_alert: showAlert });
      } catch {}
    };
    ...
    try {
      ...
      if (data.startsWith("lang:set:")) {
        ...
        if (!langCompany || this.isCompanyAccessBlocked(langCompany)) {
          await (ctx as any).reply(langT.suspended);
          return;
        }
        ...
        if (langDealer) {
          if (langDealer.isBlocked) {
            await (ctx as any).reply(langT.blocked);
            return;
          }
          if (!langDealer.isApproved) {
            ...
            await (ctx as any).reply(pending ? langT.pendingApproval : langT.accessDenied);
            return;
          }
          const limits = await this.planLimits.getLimitsForCompany(companyId);
          const storeUrl = `${this.getPublicStoreBaseUrl()}/store/${langCompany.slug || companyId}`;
          await (ctx as any).reply(
            `👋 *${langDealer.name}*${langT.loginSuccess}${langT.commands}`,
            { parse_mode: "Markdown", reply_markup: this.buildMainMenuKeyboard(langT) }
          );
          if (limits.allowWebStore) {
            await (ctx as any).reply(`🛍 ${langCompany.name}`, {
              reply_markup: {
  ...
  private async handleCart(ctx: Context, companyId: string, editMsg = false) {
    const dealer = await this.getDealerByChatId(ctx, companyId);
    if (!dealer) return;
    ...
    if (cart.size === 0) {
      if (editMsg) {
        try {
          await (ctx as any).editMessageText(t.cartEmpty, { reply_markup: backOpts });
          return;
        } catch {}
      }
  ...
  async applyBotBranding(botToken: string, companyId: string) {
    ...
    try {
      await (tg as any).callApi("setMyName", { name });
      results.name = "ok";
    } catch (e: any) {
      results.name = e?.message ?? "err";
    }
    try {
      await (tg as any).callApi("setMyDescription", { description: descriptionUz });
      results.description = "ok";
    } catch (e: any) {
      results.description = e?.message ?? "err";
    }
    try {
      await (tg as any).callApi("setMyShortDescription", { short_description: shortDescUz });
      results.shortDescription = "ok";
    } catch (e: any) {
      results.shortDescription = e?.message ?? "err";
    }
    try {
      await (tg as any).callApi("setMyCommands", { commands });
      results.commands = "ok";
    } catch (e: any) {
      results.commands = e?.message ?? "err";
    }

    // Set profile photo from company logo, fallback to Supplio default
    try {
      const appBase = (process.env.APP_URL || "http://localhost:5000").replace(/\/+$/, "");
      const logoPath = (company as any).logo
        ? String((company as any).logo)
        : null;
    ...
    // Set global web app menu button for all chats
    try {
      await (tg as any).callApi("setChatMenuButton", {
        menu_button: {
          type: "web_app",
          text: "🛍 Web Do'kon",
          web_app: { url: storeUrl },
    ...
    this.logger.log(
      `Bot branding applied for ${name}: ${JSON.stringify(results)}`
    );
    return results;
  }
  ...
  async createBot(
    ...
    try {
      ...
      if (e.code === "P2002") {
        // Token already exists — find the existing record
        const stale = await this.prisma.customBot.findFirst({
          where: { token: data.token.trim() },
        });
        if (stale) {
          // Same company: restore/reinit without error (handles cases where init failed or delete was partial)
        ...
        throw new BadRequestException("Bot token conflict. Please try again.");
      }
      if (e.code === "P2003") {
        throw new BadRequestException(
  ...
  async removeBot(id: string, companyId: string) {
    ...
    if (existing) {
      ...
      try {
        const temp = new Telegraf(bot.token);
  ...
  async adminHardDeleteBot(botId: string) {
    ...
    // Delete webhook — use in-memory instance if available, otherwise create a temp one
    const existing = this.bots.get(botId);
    if (existing) {
      try {
        await existing.telegram.deleteWebhook({ drop_pending_updates: true });
  ...
  async adminUpdateBot(
    ...
    if (updateData.isActive === false) {
      ...
      // Always re-apply branding (bio, about, profile photo, commands)
      this.applyBotBranding(updated.token, updated.companyId).catch((e) =>
        this.logger.warn(`applyBotBranding failed on adminUpdate: ${e?.message}`)
      );
    }
  ...
  async adminDeleteAllBots() {
    ...
    for (const record of allBots) {
      ...
      if (running) {
        try { await running.telegram.deleteWebhook({ drop_pending_updates: true }); } catch {}
