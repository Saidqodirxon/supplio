...
async function main() {
  ...
  const tariffData = [
    {
      ...
      priceMonthly: "0",
      priceYearly: "0",
      featuresUz: ["1 filial", "5 xodim (rol boshqaruvi)", "50 dealer", "200 mahsulot", "Web do'kon"],
      featuresRu: ["1 филиал", "5 сотрудников (роли)", "50 дилеров", "200 товаров", "Веб-магазин"],
      featuresEn: ["1 branch", "5 staff (role management)", "50 dealers", "200 products", "Web store"],
      featuresTr: ["1 şube", "5 personel (rol yönetimi)", "50 bayi", "200 ürün", "Web mağaza"],
      featuresUzCyr: ["1 филиал", "5 ходим (рол бошқаруви)", "50 дилер", "200 маҳсулот", "Веб дўкон"],
    ...
    },
    {
      ...
      priceMonthly: "99000",
      priceYearly: "990000",
      featuresUz: ["3 filial", "15 xodim + maxsus rollar", "500 dealer", "1 000 mahsulot", "Analitika", "Telegram bot"],
      featuresRu: ["3 филиала", "15 сотрудников + роли", "500 дилеров", "1 000 товаров", "Аналитика", "Telegram бот"],
      featuresEn: ["3 branches", "15 staff + custom roles", "500 dealers", "1,000 products", "Analytics", "Telegram bot"],
      featuresTr: ["3 şube", "15 personel + özel roller", "500 bayi", "1.000 ürün", "Analitik", "Telegram botu"],
      featuresUzCyr: ["3 филиал", "15 ходим + махсус роллар", "500 дилер", "1 000 маҳсулот", "Аналитика", "Telegram бот"],
    ...
    },
    {
      ...
      priceMonthly: "299000",
      priceYearly: "2990000",
      featuresUz: ["10 filial", "50 xodim + cheksiz maxsus rollar", "5 000 dealer", "Cheksiz mahsulot", "Analitika Pro", "Bir nechta bot", "Ommaviy import"],
      featuresRu: ["10 филиалов", "50 сотрудников + роли без лимита", "5 000 дилеров", "Безлимит товаров", "Аналитика Pro", "Несколько ботов", "Массовый импорт"],
      featuresEn: ["10 branches", "50 staff + unlimited custom roles", "5,000 dealers", "Unlimited products", "Pro Analytics", "Multiple bots", "Bulk import"],
      featuresTr: ["10 şube", "50 personel + sınırsız özel roller", "5.000 bayi", "Sınırsız ürün", "Pro Analitik", "Çoklu bot", "Toplu içe aktarma"],
      featuresUzCyr: ["10 филиал", "50 ходим + чексиз махсус роллар", "5 000 дилер", "Чексиз маҳсулот", "Аналитика Pro", "Кўп бот", "Оммавий импорт"],
    ...
    },
    {
      ...
      priceMonthly: "0",
      priceYearly: "0",
      featuresUz: ["Cheksiz filial", "Cheksiz xodim + maxsus rollar", "Cheksiz dealer", "Cheksiz mahsulot", "Dedicated server", "SLA 99.9%", "Texnik yordam 24/7"],
      featuresRu: ["Безлимит филиалов", "Безлимит сотрудников + роли", "Безлимит дилеров", "Безлимит товаров", "Выделенный сервер", "SLA 99.9%", "Поддержка 24/7"],
      featuresEn: ["Unlimited branches", "Unlimited staff + custom roles", "Unlimited dealers", "Unlimited products", "Dedicated server", "SLA 99.9%", "24/7 support"],
      featuresTr: ["Sınırsız şube", "Sınırsız personel + özel roller", "Sınırsız bayi", "Sınırsız ürün", "Özel sunucu", "SLA 99.9%", "7/24 destek"],
      featuresUzCyr: ["Чексиз филиал", "Чексиз ходим + махсус роллар", "Чексиз дилер", "Чексиз маҳсулот", "Dedicated server", "SLA 99.9%", "24/7 yordam"],
  ...
  */
  ...
  const testimonials = [
    {
      ...
      roleTitle: "Owner",
      contentUz:
        "Supplio bizning tarqatish tarmog'imizni to'liq avtomatlashtirdi. Endi barcha dilerlarimizni bitta paneldan boshqaramiz.",
      contentRu:
        "Supplio полностью автоматизировал нашу дистрибьюторскую сеть. Теперь управляем всеми дилерами из одной панели.",
      contentEn:
        "Supplio fully automated our distribution network. We now manage all our dealers from a single panel.",
      contentTr:
        "Supplio dağıtım ağımızı tamamen otomatize etti. Artık tüm bayilerimizi tek bir panelden yönetiyoruz.",
      rating: 5,
      order: 0,
      isActive: true,
    },
    {
      ...
      contentUz:
        "Telegram bot orqali dilerlarimiz buyurtma berishi va qarzlarini ko'rishi bizga juda qulaylik yaratdi.",
      contentRu:
        "Telegram бот для дилеров и отслеживание долгов значительно упростили нашу работу.",
      contentEn:
        "The Telegram bot for dealers and debt tracking greatly simplified our operations.",
      contentTr:
        "Bayiler için Telegram botu ve borç takibi iş süreçlerimizi büyük ölçüde kolaylaştırdı.",
      rating: 5,
      order: 1,
      isActive: true,
    },
    {
      ...
      contentUz:
        "Analitika sahifasi bizga qaysi mahsulotlar yaxshi sotilayotganini va qaysi dilerlar faolroq ekanini ko'rsatadi.",
      contentRu:
        "Раздел аналитики показывает нам, какие продукты продаются лучше и кто из дилеров активнее.",
      contentEn:
        "The analytics section shows us which products sell best and which dealers are most active.",
      contentTr:
        "Analitik bölümü hangi ürünlerin daha iyi sattığını ve hangi bayilerin daha aktif olduğunu gösteriyor.",
      rating: 5,
      order: 2,
      isActive: true,
    },
  ...
  for (const [i, test] of testimonials.entries()) {
    const existing = await (prisma as any).testimonial.findFirst({
      where: { name: test.name },
    });
    if (!existing) {
      await (prisma as any).testimonial.create({ data: test });
    }
  }
  ...
  if (existingCompany) {
    console.log(
      "Company already exists, skipping company/user/dealer creation."
    );
  ...
  const dealers = await Promise.all(
    dealerData.map((d) =>
      prisma.dealer.create({ data: { ...d, companyId: company.id } })
    )
  );

  // ── 10. Orders + Ledger (spread over 30 days) ─────────────────────────────
  ...
  for (const s of orderScenarios) {
    ...
    const order = await prisma.order.create({
      data: {
        ...
        totalAmount,
        totalCost,
        status: s.daysBack > 7 ? OrderStatus.DELIVERED : OrderStatus.PENDING,
        items: [
          {
    ...
    // Update dealer's currentDebt (simplified)
  ...
  // ── 11. Payments (some dealers have paid back) ────────────────────────────
  const paymentData = [
  ...
  console.log(
    `Dealers: ${dealers.length} | Orders: ${orderScenarios.length} | Leads: 10`
  );
}
