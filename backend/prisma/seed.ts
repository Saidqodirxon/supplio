...
async function main() {
  ...
  */
  ...
  for (const [i, test] of testimonials.entries()) {
    const existing = await (prisma as any).testimonial.findFirst({ where: { name: test.name } });
    if (!existing) {
  ...
  await prisma.user.createMany({
    data: [
      { phone: "+998901112233", passwordHash, roleType: RoleType.SUPER_ADMIN, fullName: "Super Admin", companyId: company.id },
      { phone: "+998901234567", passwordHash, roleType: RoleType.OWNER, fullName: "Aziz Karimov", companyId: company.id },
      { phone: "+998907654321", passwordHash, roleType: RoleType.MANAGER, fullName: "Dilnoza Umarova", companyId: company.id, branchId: mainBranch.id },
      { phone: "+998909876543", passwordHash, roleType: RoleType.SALES, fullName: "Bobur Toshmatov", companyId: company.id, branchId: mainBranch.id },
    ],
  });
  ...
  for (const s of orderScenarios) {
    ...
    await prisma.ledgerTransaction.create({
      data: { companyId: company.id, dealerId: dealer.id, type: TxType.ORDER, amount: totalAmount, createdAt },
    });
  ...
  for (const p of paymentData) {
    ...
    await prisma.ledgerTransaction.create({
      data: { companyId: company.id, dealerId: dealer.id, type: TxType.PAYMENT, amount: p.amount },
    });
    await prisma.dealer.update({
      where: { id: dealer.id },
  ...
  await prisma.user.create({
    data: { phone: "+998991112233", passwordHash, roleType: "OWNER" as RoleType, fullName: "Demo Owner", companyId: demoCompany.id, branchId: demoBranch.id },
  });
