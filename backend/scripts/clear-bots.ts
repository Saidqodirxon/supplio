...
async function clearBot(token: string, label: string) {
  ...
  console.log(`   getUpdates(drain) → ${upd.ok ? '✅' : '❌ ' + upd.description}`);

  // 3. Remove default menu button (reset to commands list)
  const mb = await tgCall(token, 'setChatMenuButton', { menu_button: JSON.stringify({ type: 'default' }) });
...
async function main() {
  ...
  if (SINGLE_TOKEN) {
    await clearBot(SINGLE_TOKEN, `manual token: ${SINGLE_TOKEN.slice(0, 8)}...`);
  } else {
    ...
    if (bots.length === 0) {
      ...
      for (const bot of bots) {
        await clearBot(bot.token, `${bot.company.name} — @${bot.username || bot.id.slice(-6)} [${bot.isActive ? 'active' : 'inactive'}${bot.deletedAt ? ', deleted' : ''}]`);
      }
    }
