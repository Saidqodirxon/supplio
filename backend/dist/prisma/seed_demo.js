...
async function safeDeleteByCompany(table, companyId) {
    try {
...
async function hardDeleteDemoCompanyData(companyId) {
    ...
    for (let i = 1; i <= 8; i++) {
        await prisma.user.create({
            data: {
                ...
                roleType: i <= 2 ? client_1.RoleType.MANAGER : client_1.RoleType.SALES,
                companyId: company.id,
