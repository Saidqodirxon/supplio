...
export default function WebStorePage() {
  ...
  const [form, setForm] = useState<WebStoreData>({
    website: "",
    instagram: "",
    telegram: "",
    workingHours: "",
  ...
  const storeUrl = company
    ? `${getPublicStoreBaseUrl()}/store/${company.slug}`
    : "";
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const storeOrders = orders.filter(
        (o: { source?: string; channel?: string }) =>
          o.source === "STORE" || o.channel === "STORE"
      );
      const revenue = storeOrders.reduce(
        (s: number, o: { totalAmount?: number | string }) =>
  ...
  const toggleStore = async () => {
    if (!company) return;
    try {
      const res = await api.patch("/company/me", {
        siteActive: !company.siteActive,
      });
      setCompany(res.data);
      toast.success(
        res.data.siteActive ? "Do'kon yoqildi" : "Do'kon o'chirildi"
      );
    } catch {
      toast.error(t.common.error);
    }
  ...
  const downloadQR = async () => {
    try {
      ...
      a.remove();
      toast.success(
        language === "uz" ? "QR Kod yuklab olindi" : "QR Code downloaded"
      );
    } catch {
      toast.error(t.common.error);
    }
  ...
  return (
    <div className="space-y-10 pb-10">
      ...
      >
        <div className="space-y-2">
          ...
          <p className="text-slate-500 font-bold tracking-tight">
            {language === "uz"
              ? "Onlayn katalog va mijozlar uchun buyurtma sahifasi"
              : language === "ru"
                ? "Онлайн-каталог и страница заказов для клиентов"
                : language === "tr"
                  ? "Çevrimiçi katalog ve müşteriler için sipariş sayfası"
      ...
      <motion.div
        ...
        className="bg-white dark:bg-white/5 rounded-[2.5rem] border border-slate-100 dark:border-white/5 p-8 space-y-6"
      >
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
          <div className="space-y-1">
            ...
            <p className="text-sm font-bold text-slate-400">
              {company.siteActive
                ? language === "uz"
                  ? "Do'koningiz faol — mijozlar buyurtma bera oladi"
                  : language === "ru"
                    ? "Магазин активен — клиенты могут делать заказы"
                    : language === "tr"
                      ? "Mağaza aktif — müşteriler sipariş verebilir"
                      : "Store is live — customers can place orders"
                : language === "uz"
                  ? "Do'kon o'chirilgan — hech kim kira olmaydi"
                  : language === "ru"
                    ? "Магазин отключён — доступ закрыт"
                    : language === "tr"
                      ? "Mağaza kapalı — erişim yok"
                      : "Store is disabled — no one can access it"}
            </p>
          </div>
          <button
            onClick={toggleStore}
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center gap-3 bg-slate-50 dark:bg-white/5 border border-slate-100 dark:border-white/10 rounded-2xl px-5 py-4">
            ...
            <button
              onClick={copyUrl}
              className="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-white/10 transition-all shrink-0"
            ...
            <button
              ...
              className="p-2 rounded-xl hover:bg-slate-100 dark:hover:bg-white/10 transition-all shrink-0"
              title={
                language === "uz" ? "QR kodni yuklab olish" : "Download QR Code"
              }
            >
        ...
        <div className="flex items-center gap-3 p-4 bg-blue-600/5 border border-blue-600/10 rounded-2xl">
          ...
          <p className="text-sm font-bold text-blue-700 dark:text-blue-400">
            {language === "uz"
              ? "Ushbu manzilni dilerlaringizga yuboring yoki QR kod chiqarib bering."
              : language === "ru"
                ? "Отправьте этот адрес своим дилерам или распечатайте QR-код."
                : language === "tr"
                  ? "Bu adresi bayilerinize gönderin veya QR kodu yazdırın."
      ...
      >
        {statCards.map((s, i) => (
          <div
            key={i}
            className="bg-white dark:bg-white/5 rounded-[2rem] border border-slate-100 dark:border-white/5 p-7 flex items-center gap-5"
          >
            <div
      ...
      >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
          {[
            ...
            {
              key: "adminLogBotToken",
              label:
                language === "uz"
                  ? "Loglar uchun Bot Token"
                  : language === "ru"
                    ? "Bot Token для логов"
                    : language === "tr"
                      ? "Loglar İçin Bot Token"
                      : "Log Bot Token",
              icon: Bot,
      ...
      {(company.subscriptionPlan === "FREE" ||
        company.subscriptionPlan === "START") && (
        <motion.div
          {...fadeInUp}
          transition={{ delay: 0.2 }}
          className="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-900/50 rounded-[2rem] p-6 flex items-start gap-4"
        >
          ...
          <div>
            <p className="font-black text-amber-800 dark:text-amber-400 text-sm mb-1">
              {language === "uz"
                ? "Veb do'kon PRO tarifida to'liq ochiladi"
                : language === "ru"
                  ? "Полный функционал веб-магазина доступен на тарифе PRO"
                  : language === "tr"
                    ? "Web mağazanın tam özelliği PRO planında açılır"
                    : "Full Web Store features unlock on PRO plan"}
            </p>
            <p className="text-amber-700 dark:text-amber-500 text-xs font-bold">
              {language === "uz"
                ? "Hozirgi rejada asosiy funksiyalar mavjud. Obunangizni yangilang."
                : language === "ru"
                  ? "На текущем тарифе доступны базовые функции. Обновите подписку."
                  : language === "tr"
                    ? "Mevcut planda temel özellikler mevcut. Aboneliğinizi güncelleyin."
                    : "Basic features available on current plan. Upgrade your subscription."}
            </p>
          </div>
        </motion.div>
      )}
