...
export default function Reports() {
  ...
  const t = {
    ...
    exportXlsx: isUz ? 'Excel yuklash' : isRu ? 'Скачать Excel' : 'Export Excel',
    recentOrders: isUz ? 'So\'nggi buyurtmalar' : isRu ? 'Последние заказы' : 'Recent Orders',
  ...
  const exportOverview = () => {
    ...
    downloadCsv(`hisobot-umumiy-${new Date().toISOString().split('T')[0]}.csv`, rows);
  ...
  const exportDebts = () => {
    ...
    const rows: string[][] = [
      ...
      ...debtReport.dealers.map(d => [
        d.name, d.phone, fmt(d.currentDebt), fmt(d.creditLimit), d.utilizationPercent.toFixed(1) + '%',
      ]),
    ];
    downloadCsv(`hisobot-qarzlar-${new Date().toISOString().split('T')[0]}.csv`, rows);
    toast.success('Excel tayyor!');
  };
  ...
  const exportTop = () => {
    ...
    const rows: string[][] = [
      ...
      ...topDealers.map((d, i) => [
        String(i + 1), d.name, String(d.totalOrders), fmt(d.totalAmount), fmt(d.currentDebt),
      ]),
    ];
    downloadCsv(`hisobot-top-dilerlar-${new Date().toISOString().split('T')[0]}.csv`, rows);
    toast.success('Excel tayyor!');
  };
    ...
    <div className="space-y-8">
      ...
      <div className="flex gap-2 border-b border-slate-200 dark:border-white/10 pb-1">
        {(['overview', 'debts', 'top'] as const).map(tabKey => (
          ...
          >
            {tabKey === 'overview' ? t.overview : tabKey === 'debts' ? t.debts : t.top}
          </button>
        ))}
      </div>
          ...
          {tab === 'overview' && dashData && (
            <div className="space-y-8">
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  { label: t.orders, value: dashData.stats.totalOrders.toLocaleString(), icon: ShoppingCart, color: 'text-blue-600 bg-blue-500/10' },
                  { label: t.revenue, value: fmt(dashData.stats.revenue), icon: DollarSign, color: 'text-emerald-600 bg-emerald-500/10' },
                  { label: t.profit, value: fmt(dashData.stats.profit), icon: TrendingUp, color: 'text-indigo-600 bg-indigo-500/10' },
                  { label: t.dealers, value: dashData.stats.activeDealers.toLocaleString(), icon: Users, color: 'text-violet-600 bg-violet-500/10' },
                  { label: t.products, value: dashData.stats.products.toLocaleString(), icon: BarChart3, color: 'text-amber-600 bg-amber-500/10' },
                  { label: t.debt2, value: fmt(dashData.stats.debt), icon: CreditCard, color: 'text-rose-600 bg-rose-500/10' },
                ].map(item => (
              ...
              {dashData.statusDistribution.length > 0 && (
                <div className="rounded-3xl border border-slate-100 dark:border-white/10 bg-white dark:bg-white/5 p-6">
                  <h3 className="font-black text-slate-900 dark:text-white mb-4">
                    {isUz ? 'Buyurtma holatlari' : isRu ? 'Статусы заказов' : 'Order statuses'}
                  </h3>
                  <div className="space-y-3">
                    {dashData.statusDistribution.map(s => (
                      <div key={s.status} className="flex items-center gap-3">
                        <span className={clsx('text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-lg w-28 text-center shrink-0', STATUS_COLORS[s.status] || 'text-slate-400 bg-slate-500/10')}>
                          {s.status}
                        </span>
                        <div className="flex-1 h-2 bg-slate-100 dark:bg-white/10 rounded-full overflow-hidden">
                          <div
                            className="h-full rounded-full bg-blue-500"
                            style={{ width: `${Math.min(100, (s.count / (dashData.stats.totalOrders || 1)) * 100)}%` }}
                          />
                        </div>
                        <span className="text-xs font-black text-slate-500 w-8 text-right">{s.count}</span>
                        <span className="text-xs font-bold text-slate-400 w-32 text-right hidden sm:block">{fmt(s.amount)}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {dashData.recentOrders.length > 0 && (
                <div className="rounded-3xl border border-slate-100 dark:border-white/10 bg-white dark:bg-white/5 overflow-hidden">
                  <div className="p-6 border-b border-slate-100 dark:border-white/10 flex items-center justify-between">
                    <h3 className="font-black text-slate-900 dark:text-white">{t.recentOrders}</h3>
              ...
              {debtReport ? (
                <>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {[
                      { label: t.totalDebt, value: fmt(debtReport.totalDebt), icon: TrendingDown, color: 'text-rose-600 bg-rose-500/10' },
                      { label: t.totalLimit, value: fmt(debtReport.totalCreditLimit), icon: CreditCard, color: 'text-blue-600 bg-blue-500/10' },
                      { label: t.overLimit, value: String(debtReport.overLimitCount), icon: AlertTriangle, color: 'text-amber-600 bg-amber-500/10' },
                    ].map(item => (
                      <div key={item.label} className="rounded-3xl border border-slate-100 dark:border-white/10 bg-white dark:bg-white/5 p-6">
                        <div className={clsx('w-10 h-10 rounded-2xl flex items-center justify-center mb-4', item.color)}>
                          <item.icon className="w-5 h-5" />
                  ...
                  {debtReport.dealers.length === 0 ? (
                    <div className="rounded-3xl border-2 border-dashed border-slate-200 dark:border-white/10 flex flex-col items-center justify-center py-16 gap-3">
                      ...
                      <p className="font-black text-slate-700 dark:text-slate-300">{t.noDebt}</p>
                    </div>
                  ) : (
                    <div className="rounded-3xl border border-slate-100 dark:border-white/10 bg-white dark:bg-white/5 overflow-hidden">
                      <div className="divide-y divide-slate-100 dark:divide-white/5">
                          ...
                          <div key={dealer.id} className="p-6">
                            <div className="flex items-start justify-between gap-4 mb-4">
                              ...
                              <div className="text-right">
                                ...
                                <p className="text-xs text-slate-400">{t.limit}: {fmt(dealer.creditLimit)}</p>
                              </div>
                            </div>
                            ...
                            <p className="text-xs text-slate-400 mt-1">{dealer.utilizationPercent.toFixed(1)}% {t.utilization}</p>
                          </div>
                        ))}
                      </div>
                    </div>
          ...
          {tab === 'top' && (
            <div className="rounded-3xl border border-slate-100 dark:border-white/10 bg-white dark:bg-white/5 overflow-hidden">
              {topDealers.length === 0 ? (
                ...
                <>
                  <div className="p-6 border-b border-slate-100 dark:border-white/10 flex items-center justify-between">
                    <h3 className="font-black text-slate-900 dark:text-white">{t.top}</h3>
                    <span className="text-xs font-black text-slate-400 bg-slate-100 dark:bg-white/10 px-2.5 py-1 rounded-lg">
                  ...
                  <div className="divide-y divide-slate-100 dark:divide-white/5">
                    {topDealers.map((dealer, i) => (
                      <div key={dealer.id} className="flex items-center gap-4 px-6 py-4">
                        <span className={clsx(
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-slate-400">{dealer.totalOrders} {t.orders}</p>
                        </div>
                        <div className="text-right shrink-0">
                          <p className="font-black text-slate-900 dark:text-white text-sm">{fmt(dealer.totalAmount)}</p>
                          {dealer.currentDebt > 0 && (
                            <p className="text-xs text-rose-500 font-bold mt-0.5">{t.debt}: {fmt(dealer.currentDebt)}</p>
                          )}
                        </div>
                      </div>
                    ))}
