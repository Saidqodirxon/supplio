...
function BarChart({ data }: { data: ChartPoint[] }) {
  ...
  if (!data.length)
    return (
      <div className="flex-1 flex items-center justify-center text-slate-300 dark:text-slate-700 text-xs font-bold uppercase tracking-widest">
        No data
      </div>
    );
...
function DonutChart({ data }: { data: StatusDist[] }) {
  ...
  if (!data.length)
    return (
      <div className="flex items-center justify-center h-48 text-slate-300 dark:text-slate-700 text-xs font-bold uppercase tracking-widest">
        No data
      </div>
    );
    ...
    <div className="flex flex-col items-center gap-4">
      ...
      <div className="flex flex-wrap gap-2 justify-center">
        {slices.map((s, i) => (
          <span
            key={i}
            className="flex items-center gap-1 text-[10px] font-bold text-slate-500 uppercase tracking-wider cursor-pointer"
            onMouseEnter={() => setHovered(i)}
            onMouseLeave={() => setHovered(null)}
          >
            <span
      ...
      <div className="space-y-10 animate-pulse p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="h-32 bg-slate-100 dark:bg-slate-900 rounded-[2rem]"
            />
          ))}
        </div>
        ...
        {kpiCards.map((card, idx) => (
          ...
          >
            <div className="flex justify-between items-start mb-5">
              <div
                className={`w-11 h-11 rounded-2xl ${card.bg} ${card.color} flex items-center justify-center`}
              >
            ...
            <p className="text-2xl font-black text-slate-900 dark:text-white tracking-tighter">
              {fmt(card.value)}{" "}
              <span className="text-xs opacity-30 font-bold uppercase ml-1">
                {t.common.uzs}
              </span>
            </p>
            <p className="text-[9px] text-slate-400 font-bold uppercase tracking-wider mt-2 opacity-70">
              {card.sub}
            </p>
          </motion.div>
        ))}
        ...
        {[
          {
            ...
            value: fmt(stats?.periodRevenue ?? 0),
            icon: DollarSign,
            color: "text-blue-500",
          },
          {
            label: t.dashboard.periodProfit,
            value: fmt(stats?.periodProfit ?? 0),
            icon: TrendingUp,
            color: "text-emerald-500",
          },
          {
            label: t.dashboard.periodOrders,
            value: String(stats?.periodOrders ?? 0),
            icon: ShoppingCart,
            ...
            <div className="space-y-4">
              {topDealers.map((dealer, i) => {
                const pct = Math.round(
                  (dealer.totalAmount / maxDealerAmount) * 100
                );
                const debtPct =
                  dealer.creditLimit > 0
                    ? Math.min(
                        100,
                        Math.round(
                          (dealer.currentDebt / dealer.creditLimit) * 100
                        )
                      )
                    : 0;
                return (
                    ...
                    {debtPct > 0 && (
                      <div className="flex items-center gap-2">
                        <span className="text-[9px] text-slate-400 font-bold uppercase">
                          {t.dashboard.debtPercent} {debtPct}%
                        </span>
